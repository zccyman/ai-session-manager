...
export function MessageThread({ messages, loading }: MessageThreadProps) {
  if (loading) return <div className="flex justify-center py-12"><div className="w-6 h-6 border-2 border-[#3370FF] border-t-transparent rounded-full animate-spin" /></div>;
  if (!messages || messages.length === 0) return <div className="text-center py-12 text-[#8F959E]">选择一个会话查看内容</div>;
